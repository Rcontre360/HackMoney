...
const Step1 = ({selectOrgType, selectStep}) => {
	...
	return (
		<div className="flex flex-col gap-4 items-center justify-center h-full">
			<button onClick={() => { selectOpt('new') }} className='btn bg-color1 flex flex-col items-center h-[100px]'>
				<p className='text-white text-xl'>Create an organization</p>
				<p className='text-white text-xl'>Start your organization with Aragon</p>
			</button>
			<button onClick={() => { selectOpt('existing') }} className='btn bg-color1 flex flex-col items-center'>
				...
				<div className="grid gap-4 grid-cols-3 grid-rows-2 m-10">
					{buttons.map((k, i) => (
						<button
							...
							className={clsx("btn text-white bg-color1", (orgTemplate === k.value) && "bg-color2")}
						>
							{k.title}
						</button>
					))}
...
const Step4 = ({ setTokenDetails, selectStep }) => {
	...
	return (
		<div className="flex flex-col items-center justify-center h-full">
			<div className="flex flex-col gap-4 card w-96 bg-color2 text-primary-content overflow-scroll mb-10">

				<div className="sticky top-0 bg-color1 w-full px-10 py-5">
					<h3 className='text-white text-center'>CHOOSE YOUR TOKENS SETTINGS BELOW</h3>
						...
						<div key={i} className="flex flex-row justify-around">
							<form className="w-full" variant="filled">
								...
								<input
									...
									value={k.value}
									onChange={(e) => { k.setFn(e.target.value) }}
								/>
							</form>
						</div>
					)}
	...
	return (
		<div className="flex flex-col gap-4 items-center justify-center h-full">
				<div>
					...
					<p className='text-white text-center text-2xl'>Your organization is ready</p>
				</div>
				<button className='btn bg-color1' onClick={() => { setComplete(true) }}>
